...
function load(){
  try{
    ...
    const merged={...base,...saved,version:3};
    merged.ecology={...base.ecology,...(saved.ecology||{})};
    merged.stakeholders={...base.stakeholders,...(saved.stakeholders||{})};
    merged.activePolicies=saved.activePolicies||{};
    merged.policyPipeline=saved.policyPipeline||[];
    merged.policyHistory=saved.policyHistory||[];
    merged.incidentHistory=saved.incidentHistory||[];
    if(saved.version!==3&&merged.phase==="prep"&&merged.round===0)merged.phase="governance";
    return merged;
...
function apply(delta){
  state.fanTrust=clamp(state.fanTrust+(delta.fan||0));
  state.politicalCapital=clamp(state.politicalCapital+(delta.politics||0));
  state.treasury=Math.max(0,state.treasury+(delta.treasury||0));
  state.coachTrust=clamp(state.coachTrust+(delta.coach||0));
  state.mediaPressure=clamp(state.mediaPressure+(delta.media||0));
  state.clubRelations=clamp(state.clubRelations+(delta.club||0));
  state.fatigue=clamp(state.fatigue+(delta.fatigue||0));
}
function metricLabel(v){return v>=65?"稳固":v>=45?"尚可":"承压";}
function pressureLabel(v){return v>=65?"舆论高压":v>=42?"持续关注":"压力有限";}
function readiness(){
  const c=state.choices;
  const match=(options.prep[c.prep]?.readiness||0)+(options.release[c.release]?.readiness||0)+(options.mandate[c.mandate]?.readiness||0);
  return match+(typeof policyReadinessBonus==="function"?policyReadinessBonus():0);
}
...
function simulateMatch(){
  ...
  const f=currentFixture();const home=f.venue==="home";
  const teamStrength=70+readiness()+(state.coachTrust-50)/18-(state.fatigue-25)/22-(state.mediaPressure-40)/45;
...
function simulateOtherMatches(){
  ...
  pairs.forEach(([a,b],i)=>{const ta=state.table[a],tb=state.table[b];const noise=(seed(`${state.campaign}-${round}-${i}`)-.5)*2;const diff=(ta.strength-tb.strength)/12+noise;const ga=Math.max(0,Math.min(4,Math.floor(1.2+diff*.35+seed(`${a}-${round}`)*1.5)));const gb=Math.max(0,Math.min(4,Math.floor(1.2-diff*.3+seed(`${b}-${round}`)*1.5)));applyTable(a,b,ga,gb);});
}
...
function confirmAction(){
  ...
  if(state.phase==="prep"){const o=options.prep[state.selected];state.choices.prep=state.selected;apply(o);record(`主席批准“${o.title}”。`);state.phase=nextAfterPrep();}
    ...
    const o=options.review[state.selected];state.choices.review=state.selected;apply(o);record(`主席签署赛后决定：“${o.title}”。`);
    if(o.dismiss){const outgoing=state.coachName;state.coachIndex=(state.coachIndex+1)%coachNames.length;state.coachName=coachNames[state.coachIndex];state.coachTrust=48;state.coachStatus="新任过渡期";state.coachHistory.push({campaign:state.campaign,round:state.round+1,outgoing,incoming:state.coachName});record(`${outgoing}离任，${state.coachName}接任国家队主教练。`);}else state.coachStatus=state.selected==="technical"?"专项复盘中":"获主席公开支持";
    state.phase="between";
  }
