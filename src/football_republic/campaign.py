...
STRATEGIES: dict[Strategy, PresidentialPlan] = {
    Strategy.FOUNDATIONS: PresidentialPlan(18_000_000, 16_000_000, 8_000_000, 0.72, 3_000_000, 4_000_000),
    Strategy.BALANCED: PresidentialPlan(12_000_000, 10_000_000, 7_000_000, 0.58, 2_500_000, 14_000_000),
    Strategy.QUICK_RESULTS: PresidentialPlan(5_000_000, 4_000_000, 2_000_000, 0.35, 1_000_000, 32_000_000),
}
...
class Campaign:
    def __init__(self, engine: SimulationEngine | None = None) -> None:
        ...
        self.initial_youth_environment = self.engine.state.youth_development_environment
        self.dashboards: list[Dashboard] = [self.dashboard()]
    ...
    def enact_plan(self, plan: PresidentialPlan) -> dict[str, object]:
        ...
        coach_report = self.engine.enact_coach_education_grant(
            CoachEducationGrant(
                ...
                requested_trainees={"coast": 380, "heartland": 520, "frontier": 300},
            )
        )
        match_report = self.engine.enact_youth_match_grant(
            YouthMatchGrant(
                ...
                requested_player_slots={"coast": 70_000, "heartland": 95_000, "frontier": 45_000},
            )
        )
        school_success = self.engine.negotiate_school_football_agreement(plan.school_cofunding)
    ...
    def run(self, months: int = 24) -> BoardReview:
        ...
        for _ in range(months):
            ...
            if self.engine.state.month % 6 == 0:
                self.dashboards.append(self.dashboard())
    ...
    def board_review(self) -> BoardReview:
        state = self.engine.state
        youth_change = state.youth_development_environment - self.initial_youth_environment
        youth_component = max(0.0, min(30.0, 12.0 + youth_change * 4.5))
        club_component = 20.0 * state.solvent_club_share
        trust_component = 18.0 * state.fan_trust
        integrity_component = 12.0 * state.integrity_reputation
        team_component = max(0.0, min(15.0, 7.0 + (state.national_team_strength - 47.5) * 1.6))
        capital_component = 5.0 * state.political_capital
        score = youth_component + club_component + trust_component + integrity_component + team_component + capital_component
        if score >= 67:
