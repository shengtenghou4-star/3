...
Every object in this layer reflects a real dossier, contact, mandate, official, meeting
or press exchange. It never exposes hidden simulation scores.
"""
...
def inject_cinematic_theme() -> None:
    st.markdown(
        ...
        <style>
        :root {
          --fr-bg:#071019; --fr-panel:rgba(15,29,42,.86); --fr-line:rgba(168,191,211,.17);
          --fr-gold:#d9b96d; --fr-red:#a33b38; --fr-green:#65b98b; --fr-blue:#6fa8d5;
          --fr-text:#edf3f7; --fr-muted:#94a7b7; --fr-shadow:0 26px 70px rgba(0,0,0,.34);
        }
        ...
        .stApp {
          background:radial-gradient(circle at 72% 8%,rgba(58,91,118,.18),transparent 27%),
        ...
        .stApp::before {
          content:"";position:fixed;inset:0;pointer-events:none;opacity:.028;z-index:0;
        ...
        [data-testid="stSidebar"] {
          background:linear-gradient(180deg,rgba(17,34,49,.98),rgba(8,18,27,.99))!important;
          border-right:1px solid rgba(217,185,109,.18)!important;box-shadow:16px 0 50px rgba(0,0,0,.22);
        }
        [data-testid="stSidebar"] .stButton>button,[data-testid="stSidebar"] .stDownloadButton>button {
          border:1px solid rgba(217,185,109,.22);background:rgba(255,255,255,.045);color:#eef3f6;
        }
        [data-testid="stSidebar"] .stButton>button:hover,[data-testid="stSidebar"] .stDownloadButton>button:hover {
          border-color:rgba(217,185,109,.66);background:rgba(217,185,109,.09);transform:translateY(-1px);
        }
        .block-container {max-width:1550px;padding-top:1.2rem;padding-bottom:5rem;}
        h1,h2,h3,h4 {letter-spacing:-.035em;} h3 {color:#f0f5f8;} hr {border-color:rgba(112,137,157,.22)!important;}

        [data-testid="stTabs"] [data-baseweb="tab-list"] {
          gap:7px;overflow-x:auto;padding:7px;margin:4px 0 20px;border:1px solid rgba(168,191,211,.14);
          border-radius:16px;background:rgba(9,20,30,.72);box-shadow:inset 0 1px 0 rgba(255,255,255,.035),0 12px 34px rgba(0,0,0,.18);backdrop-filter:blur(14px);
        }
        [data-testid="stTabs"] button[role="tab"] {min-height:42px;border-radius:11px;color:#93a6b6;white-space:nowrap;transition:all .18s ease;}
        [data-testid="stTabs"] button[role="tab"]:hover {color:#f0f4f7;background:rgba(255,255,255,.045);}
        [data-testid="stTabs"] button[role="tab"][aria-selected="true"] {
          color:#101820;background:linear-gradient(180deg,#e4c87e,#cba85a);font-weight:780;box-shadow:0 8px 20px rgba(203,168,90,.22);
        }
        [data-testid="stTabs"] [data-baseweb="tab-highlight"] {display:none;}

        .stButton>button,.stDownloadButton>button {
          min-height:44px;border-radius:11px;border:1px solid rgba(135,159,180,.27);
          background:linear-gradient(180deg,rgba(27,48,66,.95),rgba(15,31,45,.95));color:#eef4f7;font-weight:720;
          box-shadow:0 8px 22px rgba(0,0,0,.18),inset 0 1px 0 rgba(255,255,255,.045);transition:transform .16s ease,border-color .16s ease;
        }
        .stButton>button:hover,.stDownloadButton>button:hover {transform:translateY(-2px);border-color:rgba(217,185,109,.72);color:#fff;}
        .stButton>button[kind="primary"] {color:#101820;border-color:#e2c577;background:linear-gradient(180deg,#e8cd86,#caa657);box-shadow:0 12px 30px rgba(202,166,87,.24);}
        ...
        [data-baseweb="select"]>div,[data-baseweb="input"]>div,[data-baseweb="textarea"]>div {
          color:#edf3f7;border-color:rgba(139,164,184,.27)!important;background:rgba(11,24,36,.88)!important;border-radius:11px!important;
        }
        ...
        [data-testid="stExpander"] {border:1px solid rgba(137,161,181,.18)!important;border-radius:13px!important;background:rgba(13,27,40,.62)!important;overflow:hidden;}
        ...
        .office-cinema {
          position:relative;overflow:hidden;min-height:238px;display:grid;grid-template-columns:118px minmax(0,1.5fr) minmax(280px,.8fr);
          gap:24px;align-items:center;margin:0 0 13px;padding:25px 29px;border:1px solid rgba(217,185,109,.24);border-radius:22px;
          background:linear-gradient(112deg,rgba(25,48,66,.96) 0%,rgba(11,25,37,.97) 50%,rgba(8,18,28,.98) 100%);
          box-shadow:var(--fr-shadow),inset 0 1px 0 rgba(255,255,255,.045);
        }
        .office-cinema::before {content:"";position:absolute;inset:0;pointer-events:none;background:radial-gradient(circle at 14% 20%,rgba(217,185,109,.13),transparent 24%),linear-gradient(90deg,transparent 0 74%,rgba(255,255,255,.018) 74% 74.3%,transparent 74.3% 100%);}
        .office-cinema::after {content:"";position:absolute;right:-75px;bottom:-115px;width:330px;height:330px;border:1px solid rgba(217,185,109,.15);border-radius:50%;box-shadow:0 0 0 34px rgba(217,185,109,.025),0 0 0 68px rgba(217,185,109,.016);}
        .office-crest {
          position:relative;z-index:1;width:105px;aspect-ratio:1;display:grid;place-items:center;border:1px solid rgba(229,202,135,.66);border-radius:50%;
          color:#f0d995;background:radial-gradient(circle,rgba(217,185,109,.13),rgba(4,12,19,.48));box-shadow:inset 0 0 0 8px rgba(217,185,109,.035),0 14px 30px rgba(0,0,0,.32);
          font-family:Georgia,serif;font-size:2.15rem;font-weight:800;letter-spacing:-.08em;
        }
        .office-crest small {position:absolute;bottom:15px;font:650 .55rem/1 sans-serif;letter-spacing:.16em;color:#9cb0bf;}
        .office-copy {position:relative;z-index:1;} .office-eyebrow {color:#d9b96d;font-size:.69rem;font-weight:800;letter-spacing:.16em;text-transform:uppercase;}
        .office-copy h1 {margin:7px 0 8px;color:#f3f7f9;font-size:clamp(2rem,4vw,3.45rem);line-height:1;}
        .office-copy p {margin:0;color:#a6b6c3;font-size:.96rem;} .office-copy .situation {margin-top:17px;max-width:800px;color:#e6edf1;font-size:1.02rem;line-height:1.65;}
        ...
        .office-date-card .day {color:#f5f7f8;font-size:1.45rem;font-weight:780;} .office-date-card .place {margin-top:3px;color:#8fa2b1;font-size:.82rem;}
        ...
        .pressure-chip.amber::before {background:#d4ad59;box-shadow:0 0 0 4px rgba(212,173,89,.12);} .pressure-chip.red::before {background:#d56761;box-shadow:0 0 0 4px rgba(213,103,97,.12);} .pressure-chip.green::before {background:#65b98b;box-shadow:0 0 0 4px rgba(101,185,139,.12);}

        .desk-scene {
          position:relative;overflow:hidden;min-height:226px;margin:0 0 20px;padding:22px;border:1px solid rgba(128,151,171,.17);border-radius:18px;
          background:linear-gradient(180deg,rgba(7,15,23,.2),rgba(7,15,23,.75)),repeating-linear-gradient(96deg,#3b281b 0 44px,#4b3222 44px 88px,#342318 88px 132px);
        ...
        .desk-object h4 {margin:25px 0 5px;color:#f0f4f6;font-size:1rem;} .desk-object p {margin:0;color:#8fa0ad;font-size:.78rem;line-height:1.45;}
        ...
        .folder-stack::before {top:13px;transform:rotate(-1.2deg);} .folder-stack::after {top:20px;left:18px;right:9px;background:#ae3e3a;transform:rotate(.8deg);}
        ...
        .life-step.done {color:#aebdc8;border-color:rgba(101,185,139,.22);} .life-step.done::before {background:#65b98b;box-shadow:0 0 0 5px rgba(101,185,139,.10);}
        .life-step.current {color:#f0f4f6;border-color:rgba(217,185,109,.55);background:rgba(217,185,109,.07);} .life-step.current::before {background:#d9b96d;box-shadow:0 0 0 6px rgba(217,185,109,.12);animation:pulse 1.8s ease-in-out infinite;}
        .life-step.red.current {border-color:rgba(213,103,97,.55);background:rgba(213,103,97,.07);} .life-step.red.current::before {background:#d56761;box-shadow:0 0 0 6px rgba(213,103,97,.12);}
        ...
        .portrait-sigil.blue {background:linear-gradient(180deg,#8ec0e4,#537f9f);} .portrait-sigil.red {background:linear-gradient(180deg,#d98580,#98514d);} .portrait-sigil.green {background:linear-gradient(180deg,#8bcda9,#4f8e6d);} .portrait-sigil.violet {background:linear-gradient(180deg,#b39bd4,#715d91);}
        .official-copy h4 {margin:0;color:#f0f4f6;font-size:1.12rem;} .official-copy .role {color:#d9b96d;font-size:.75rem;} .official-copy .public-state {margin-top:5px;color:#91a4b3;font-size:.78rem;}
        ...
        .report-document::before {content:"";position:absolute;top:0;left:0;right:0;height:6px;background:#a7894b;} .report-document.blue::before {background:#537f9f;} .report-document.red::before {background:#98514d;} .report-document.green::before {background:#4f8e6d;} .report-document.violet::before {background:#715d91;}
        .report-document h4 {margin:11px 0 7px;color:#1d252b;font-size:1.05rem;} .report-document p {color:#3f474c;font-size:.82rem;line-height:1.52;} .report-document .doc-kicker {color:#775e2e;font-size:.64rem;font-weight:850;letter-spacing:.09em;text-transform:uppercase;}
        .report-document .doc-author {margin-top:5px;color:#6c7378;font-size:.72rem;} .report-document .doc-note {padding-top:10px;border-top:1px solid rgba(38,47,53,.16);color:#5b6267;font-size:.75rem;}
        ...
        .meeting-heads {position:relative;z-index:1;display:grid;grid-template-columns:1fr 115px 1fr;gap:22px;align-items:center;} .meeting-person {text-align:center;}
        ...
        .person-name {color:#f0f5f7;font-weight:800;} .person-role {color:#8ea1af;font-size:.74rem;} .meeting-center {text-align:center;color:#d9b96d;font-size:.72rem;letter-spacing:.1em;text-transform:uppercase;}
        ...
        .microphones {position:absolute;z-index:2;top:51px;left:50%;width:72px;height:28px;transform:translateX(-50%);} .microphones::before,.microphones::after {content:"";position:absolute;top:0;width:6px;height:38px;border-radius:5px;background:#1c252c;transform-origin:bottom;}
        .microphones::before {left:20px;transform:rotate(-22deg);} .microphones::after {right:20px;transform:rotate(22deg);}
        .reporter-row {position:relative;z-index:1;display:flex;justify-content:center;gap:18px;margin-top:15px;}
        .reporter-seat {width:34px;height:34px;border-radius:50% 50% 42% 42%;background:#243846;box-shadow:0 22px 0 8px #182a37;opacity:.86;}
        .press-topic {position:relative;z-index:1;margin-top:26px;text-align:center;color:#eef4f7;font-size:1.05rem;font-weight:780;} .press-sub {position:relative;z-index:1;margin-top:4px;text-align:center;color:#8da0af;font-size:.78rem;}
        .exchange {position:relative;width:auto;height:auto;margin:14px 0;padding:15px 18px;border-radius:13px;background:rgba(17,34,48,.78);border:1px solid rgba(140,163,183,.16);box-shadow:none;opacity:1;}
        .exchange.reporter {margin-right:8%;border-left:4px solid #6e96b4;} .exchange.president {margin-left:8%;border-left:4px solid #d9b96d;background:rgba(38,48,53,.74);}
        .exchange-label {color:#8fa5b5;font-size:.69rem;font-weight:800;letter-spacing:.1em;text-transform:uppercase;} .exchange blockquote {margin:7px 0 0;color:#edf3f6;font-size:.98rem;line-height:1.6;} .exchange-consequence {margin-top:8px;color:#8da0ae;font-size:.75rem;}
        ...
        @media (max-width:1000px) {
          .office-cinema {grid-template-columns:90px 1fr;} .office-crest {width:82px;font-size:1.65rem;} .office-date-card {grid-column:1/-1;border-left:0;border-top:1px solid rgba(217,185,109,.16);padding:15px 0 0;}
          .desk-grid {grid-template-columns:repeat(2,1fr);} .meeting-heads {grid-template-columns:1fr 80px 1fr;}
        }
        @media (max-width:660px) {
          .block-container {padding-left:.75rem;padding-right:.75rem;} .office-cinema {grid-template-columns:1fr;padding:21px;} .office-crest {display:none;} .office-copy h1 {font-size:2.15rem;}
          .desk-grid {grid-template-columns:1fr 1fr;gap:10px;} .desk-object {min-height:125px;padding:13px;} .mandate-lifecycle {grid-template-columns:1fr 1fr;}
          .meeting-heads {grid-template-columns:1fr 55px 1fr;gap:8px;} .person-head {width:58px;height:58px;} .press-stage {padding:20px 13px;} .reporter-row {gap:10px;} .reporter-seat {width:28px;height:28px;}
          .exchange.reporter,.exchange.president {margin-left:0;margin-right:0;}
        }
...
def render_desk_scene(game, packet) -> None:
    ...
    st.markdown(
        ...
        <section class="desk-scene">
          ...
          <div class="desk-grid">
            <div class="desk-object folder-stack"><span class="object-badge">{dossier_count}</span><h4>主席收文盘</h4><p>{escape(dossier_copy)}</p></div>
            <div class="desk-object phone-object"><span class="object-badge">{correspondence_count}</span><h4>红线电话</h4><p>{escape(call_copy)}</p></div>
            <div class="desk-object press-object"><span class="object-badge">{press_count}</span><h4>舆情屏</h4><p>{escape(press_copy)}</p></div>
            <div class="desk-object mandate-object"><span class="object-badge">{mandate_count}</span><h4>督办文件夹</h4><p>{escape(mandate_copy)}</p></div>
          </div>
        </section>
        """,
        unsafe_allow_html=True,
...
def render_mandate_lifecycle(mandate, status_label: str) -> None:
    ...
    st.markdown('<div class="mandate-lifecycle">' + "".join(steps) + "</div>", unsafe_allow_html=True)
    st.caption(f"当前节点：{status_label}")


def render_official_portrait(office: str, official_name: str, public_state: str) -> None:
    sigil = _OFFICE_SIGILS.get(office, official_name[:1] if official_name else "官")
    tone = _OFFICE_TONES.get(office, "gold")
    st.markdown(
        f"""
        <div class="official-portrait">
          <div class="portrait-sigil {escape(tone)}">{escape(sigil)}</div>
          <div class="official-copy"><div class="role">{escape(office)}</div><h4>{escape(official_name)}</h4><div class="public-state">{escape(public_state)}</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_report_document(report) -> None:
    ...
    st.markdown(
        ...
        <article class="report-document {escape(tone)}">
          ...
          <p><b>建议：</b>{escape(report.recommendation)}</p><p><b>材料依据：</b>{escape(report.evidence)}</p>
    ...
    st.markdown(
        ...
        <section class="meeting-room">
          <div class="meeting-heads">
            <div class="meeting-person"><div class="person-head">{escape(chairman_initial)}</div><div class="person-name">{escape(game.player_name)}</div><div class="person-role">国家足协主席</div></div>
            <div class="meeting-center">第三会客室<br>{escape(meeting.requested_duration)}</div>
            <div class="meeting-person"><div class="person-head">{escape(visitor_initial)}</div><div class="person-name">{escape(meeting.visitor)}</div><div class="person-role">{escape(meeting.institution)}</div></div>
          </div>
...
def render_press_stage(session) -> None:
    ...
    reporters = '<span class="reporter-seat"></span>' * 7
    st.markdown(
        f"""
        <section class="press-stage">
          <div class="press-banner">National Football Association · Press Room</div><div class="microphones"></div><div class="podium">FR</div>
          <div class="reporter-row">{reporters}</div><div class="press-topic">{escape(session.topic)}</div><div class="press-sub">{escape(session.outlet)} · {escape(status)}</div>
        </section>
        """,
        unsafe_allow_html=True,
    )
...
def render_press_exchange(exchange) -> None:
    st.markdown(
        f"""
        <div class="exchange reporter"><div class="exchange-label">记者 · 第{exchange.round_number}轮</div><blockquote>{escape(exchange.question)}</blockquote></div>
        <div class="exchange president"><div class="exchange-label">主席公开答复</div><blockquote>“{escape(exchange.quote)}”</blockquote><div class="exchange-consequence">{escape(exchange.consequence)}</div></div>
        """,
        unsafe_allow_html=True,
    )
